  ...
  return (
    <section ref={sectionRef} className="relative py-16 md:py-24 overflow-hidden bg-white">
      ...
      <div className="container mx-auto px-6 lg:px-8 relative z-10">
        ...
        {/* Animated Tabs */}
        <div className={`flex justify-center mb-10 ${isVisible ? 'animate-fade-in-up-delay' : 'opacity-0'}`}>
          <div className="inline-flex bg-gray-100 rounded-xl p-1.5 shadow-lg border border-gray-200">
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold text-sm transition-all duration-300 transform ${
                    activeTab === tab.id
                      ? 'bg-white text-blue-600 shadow-md scale-105'
                      : 'text-gray-600 hover:text-gray-900 hover:scale-102'
                  }`}
                >
                  <IconComponent className={`text-base transition-transform duration-300 ${activeTab === tab.id ? 'rotate-12' : ''}`} />
                  <span>{tab.label}</span>
                </button>
              );
            })}
        ...
        {/* Animated Items Grid */}
        <div className={`grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-7xl mx-auto ${isVisible ? 'animate-fade-in-up-delay-2' : 'opacity-0'}`}>
          {getActiveItems().map((item, index) => {
            ...
            return (
              <Link
                key={index}
                to={item.path}
                className="service-card group relative bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 hover:scale-105 overflow-hidden border border-gray-100"
              ...
              >
                ...
                <div className="absolute inset-0 bg-blue-500/0 group-hover:bg-blue-500/5 transition-all duration-500 rounded-xl"></div>
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    <div className={`w-12 h-12 bg-white/90 backdrop-blur-sm rounded-xl flex items-center justify-center shadow-lg transition-all duration-500 group-hover:bg-blue-600 group-hover:scale-110 group-hover:rotate-6 ${
                      hoveredIndex === index ? 'animate-pulse' : ''
                    }`}>
                      <IconComponent className="text-xl text-blue-600 group-hover:text-white transition-all duration-500" />
                    </div>
                  </div>
                  
                  {/* Arrow on Hover */}
                  <div className={`absolute bottom-4 right-4 w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center transform transition-all duration-500 ${
                ...
                {/* Content */}
                <div className="p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors duration-300 line-clamp-2 min-h-[56px]">
                    {item.title}
                  </h3>
                  
                  <p className="text-sm text-gray-600 leading-relaxed mb-4 line-clamp-2 group-hover:text-gray-700 transition-colors duration-300">
                    {item.description}
                  </p>
                  
                  {/* Learn More Link */}
                  <div className={`flex items-center gap-2 text-blue-600 font-semibold text-sm transition-all duration-300 ${
                  ...
                  {/* Bottom Accent */}
                  <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-blue-600 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></div>
                </div>

                {/* Glow Effect */}
                <div className="absolute inset-0 rounded-xl bg-blue-500/0 group-hover:bg-blue-500/10 blur-xl transition-all duration-500 -z-10"></div>
              </Link>
            );
          })}
        </div>
        ...
        <div className={`text-center mt-12 ${isVisible ? 'animate-fade-in-up-delay-3' : 'opacity-0'}`}>
          <Link
            to={activeTab === 'services' ? '/service' : activeTab === 'industries' ? '/industry' : '/technology'}
            className="group inline-flex items-center gap-3 px-8 py-4 bg-blue-600 text-white rounded-xl font-bold text-lg hover:bg-blue-700 transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:-translate-y-1 hover:scale-105"
          >
